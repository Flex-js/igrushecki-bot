...
const initCommandHandlers = (bot) => {
	...
	bot.command('wash', ctx => {
		const reply = ctx.update.message.reply_to_message;
		
		if (reply) {
			ctx.reply('💦💦💦', {reply_to_message_id: reply.message_id});
			ctx.reply('ПШШШШ ПШШШШ');
		} else {
			ctx.reply('_Используйте эту команду в ответ на сообщение, чтобы умыть собеседника_', {parse_mode: 'Markdown'});
		}
	});
};
